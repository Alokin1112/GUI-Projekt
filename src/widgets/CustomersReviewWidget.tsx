...
import {
  ReviewsDisplayed,
  changeReviewsDisplayed,
} from "../core/store/globalSettingsSlice";
...
export const CustomersReviewWidget: FunctionComponent = () => {
  ...
  const checkedReviewType = useSelector(
    (state: any) => state.globalSettings.reviewsDisplayed
  );
  const comments = useSelector(
    (state: any) => state?.user.shops[state?.user?.selectedShop].comments
  );
  const [filteredComments, setFilteredComments] = useState([] as CommentRow[]);

  const items: ToggleItem[] = [
    { id: "all", title: t("customersReview.all") },
    { id: "positive", title: t("customersReview.positive") },
  ...
  useEffect(() => {
    setFilteredComments(
      FilterComments(checkedReviewType, comments).slice(0, 5)
    );
  ...
  return (
    <>
      <CardWithTitle
        icon="star"
        title={t("customersReview.title")}
        style={{ gridArea: "customerReview" }}
        link={"/" + RoutesPath.CUSTOMER_REVIEW}
      >
        {comments?.length == 0 ? (
          <NoSuitableComments />
        ) : (
                ...
                <tbody>
                  {filteredComments?.map((item, index) => (
                    <CustomerTableRow key={index}>
                      ...
                      <TableComment title={item?.comment}>
                        {item?.comment}
                      </TableComment>
                    </CustomerTableRow>
                  ))}
